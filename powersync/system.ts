...
import { db, powersync } from "./db";
import {
  syncRoleStreams,
  unsubscribeAllRoleStreams,
} from "./streamSubscriptions";

// Re-export the instances so existing `@/powersync/system` consumers keep
// working. The actual creation lives in `./db` — a leaf module with no
// app-internal imports — so cycle members (Connector → crudMeta/syncEvents,
// streamSubscriptions) import from there directly without dragging this
// orchestration module into their graph.
export { db, powersync };
