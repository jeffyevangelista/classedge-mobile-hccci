...
export const getASItem = async <T>(key: string): Promise<T | null> => {
  try {
    ...
    if (__DEV__)
      console.log(
        `Error getting item with key ${key} from AsyncStorage`,
        error,
      );
    return null;
  }
...
export const setASItem = async (key: string, value: unknown) => {
  try {
    ...
    if (__DEV__) console.log(`Item with key ${key} set in AsyncStorage`);
  } catch (error) {
    if (__DEV__)
...
export const deleteASItem = async (key: string) => {
  try {
    ...
    if (__DEV__) console.log(`Item with key ${key} removed from AsyncStorage`);
  } catch (error) {
    if (__DEV__)
      console.log(
